...
class VideoRenderController(Controller):
    ...
    def _finish_run_loop_iteration(self,output_p) -> None:
        ...
        parent_dir = os.path.dirname(output_p)
        target_dir = f'{parent_dir}/sequence/'
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)
        # save frame as image
        if self.frames_rendered % 10 == 0:  # only save every 10th frame
            img_data = np.array(self.frame_data[::-1, :, [2, 1, 0, 3]], dtype=np.uint8)  # convert to RGBA
            img = Image.fromarray(img_data, 'RGBA')
            img.save(f'{target_dir}frame_{self.frames_rendered}.png')  # save image to current directory
